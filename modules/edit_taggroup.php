...
if (Tags::is_owned_by_user($user,Tags::get_taggroupid_name($user,$taggroupid))) {
	...
	if ($action == "edit")
	{
		...
		else
		{
				...
				elseif ($form_taggroup_doubleentry_error == true) { $taggroup_edit_message = "<font color=\"red\">(bereso_template-edit_taggroup_entry_error_name_exists)</font>"; } // text wrong char
	...
	// tag groups
		...
		{	
			...
			while ($row = $result -> fetch_assoc())
			{
				...
				$insert_hashtag = str_replace("(bereso_edit_taggroup_insert_hashtag_name)",$row['tags_name'],$insert_hashtag);
				$insert_hashtag = str_replace("(bereso_edit_taggroup_insert_hashtag_value)","#".$row['tags_name']." ",$insert_hashtag);
			}
		}
